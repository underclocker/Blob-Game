...
public class ImplodeBall extends Actor{
	...
	public ImplodeBall(Level level, long id, int suppliedTime) {
		super(level, id);
		thisID = id;
		mName = "ImplodeBall";
		mTex = "data/"+Game.get().getBodyEditorLoader().getImagePath(mName);
		...
		for(Actor a : actors) {
			...
			if(distCheckLen <= detectRadius) {
				if(a instanceof Blob){
					...
					Vector2 force = new Vector2(-implosionForce/blob.getNumSubBodies()*3, 0);
					...
					Vector2 force = new Vector2(-implosionForce, 0);
